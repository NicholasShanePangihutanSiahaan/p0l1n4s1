...
class ManualSprayMission(BaseMissionStrategy):
    @property
    def navigation_states(self) -> Sequence[str]:
        return (
            'POST_TAKEOFF_HOVER',
            'EXPLORE_ROW', 'ALIGN_TO_TREE', 'APPROACH_TREE', 'VERIFY_TREE', 'START_ORBIT', 
            'ALIGN_TO_LAST_ORBIT', 'WAIT_ORBIT', 'POST_ORBIT_HOVER', 'ALIGN_HOME', 
            'END_OF_ROW', 'DETECT_FLOWER', 'ALIGN_TO_FLOWER', 'FLOWER_ALIGNED',
    ...
    def timeout_exempt_states(self) -> Sequence[str]:
        return (
            'WAIT_START', 'EXPLORE_ROW', 'WAIT_ORBIT', 'LANDING', 'DONE',
    ...
    def execute(self, fsm: MissionStateMachine, active: bool, elapsed: float, cx: float, cy: float):
        ...
        if fsm.state == 'WAIT_START':
            if fsm.start_requested and not fsm.frame_alignment_ready:
                ...
                fsm.transition('INIT')
                fsm.get_logger().info('Start diterima dan sensor sehat; memulai preflight.')
            ...
            if fsm.is_armed:
                ...
                if elapsed_ns >= 350_000_000:
                    takeoff_msg = Float32(); takeoff_msg.data = fsm.flight_altitude
                    fsm.cmd_takeoff_pub.publish(takeoff_msg)
                    fsm.transition("WAIT_TAKEOFF")
                    fsm.retry_counter = 0
                    fsm.get_logger().info(f"Motor Bersenjata (Armed). Takeoff ke ketinggian {fsm.flight_altitude}m...")
            else:
                fsm.retry_counter += 1
                if fsm.retry_counter > 20:
                    arm_msg = Bool(); arm_msg.data = True
                    fsm.cmd_arm_pub.publish(arm_msg)
                    fsm.get_logger().info("Mencoba Arming ulang... (Menunggu Pre-arm good dari ArduPilot)")
                    fsm.retry_counter = 0
            ...
            if elapsed >= fsm.post_takeoff_hover_time:
                ...
                fsm.get_logger().info("Hover pasca-takeoff selesai. Mulai EXPLORE_ROW (mencari pohon).")
        ...
        # --- FASE MISI UTAMA ---
            ...
            if fsm.target_tree is not None:
                ...
                fsm.get_logger().info(f"Pohon ditemukan di ({fsm.target_tree.x:.1f}, {fsm.target_tree.y:.1f})")
            else:
            ...
            if fsm.yaw_aligned(fsm.current_yaw(), target_yaw, fsm.align_yaw_tolerance):
                ...
                if held >= fsm.align_yaw_hold_time:
                    ...
                    fsm.get_logger().info('Yaw ke pohon stabil; memulai translasi approach.')
            else:
            ...
            if dist_to_stop > fsm.approach_goal_tolerance:
                ...
                fsm.hover_timer = 0
                fsm.get_logger().info("Titik pengereman tercapai. Hovering 4 detik untuk stabilisasi...")
            ...
            if fsm.hover_timer >= 40:
                ...
                if target_matched_tree is not None:
                    actual_dist_to_tree = fsm.distance(cx, cy, target_matched_tree.x, target_matched_tree.y)
                    min_verify = max(0.1, fsm.approach_safe_dist - fsm.tree_distance_tolerance)
                    ...
                    if min_verify <= actual_dist_to_tree <= max_verify:
                        ...
                        if fsm.verification_retries <= fsm.verification_retry_limit:
                            ...
                            fsm.get_logger().warning(f"Pohon ID:{target_matched_tree.id} gagal verifikasi; dihapus.")
                            update_msg = Tree()
                            update_msg.id = target_matched_tree.id
                    ...
                    fsm.get_logger().warn("Pohon hilang dari peta saat hovering! Membatalkan orbit.")
                    if fsm.target_tree is not None:
                        update_msg = Tree()
            ...
            if fsm.receiving_flower_pose and not fsm.done_receiving_flower_pose:
                ...
                fsm.get_logger().info("Orbit selesai. Hover sebelum kembali ke titik takeoff.")
            ...
            if dist_to_goal <= fsm.approach_goal_tolerance and fsm.yaw_aligned(fsm.current_yaw(), target_yaw, fsm.align_yaw_tolerance):
                fsm.transition("FLOWER_ALIGNED")
                fsm.get_logger().info(f"Berhasil Aligned dengan Bunga pada ({goal_x:.2f}, {goal_y:.2f})")

        elif fsm.state == "FLOWER_ALIGNED":
            tree_x, tree_y = fsm.target_tree.x, fsm.target_tree.y
            flower_x, flower_y = fsm.flower_pose.position.x, fsm.flower_pose.position.y
            ...
            if fsm.hover_timer >= required_ticks:
                ...
                fsm.get_logger().info("Arah ke home stabil. Mulai kembali ke titik takeoff.")
            ...
            if fsm.distance(cx, cy, home_x, home_y) < MissionConfig.HOME_POSITION_TOLERANCE:
            ...
            last_command_age = (
                float('inf') if fsm.last_landing_command_time is None else
                (now - fsm.last_landing_command_time).nanoseconds * 1e-9)
            if fsm.landing_command_due(fsm.current_mode, last_command_age, fsm.landing_retry_interval):
